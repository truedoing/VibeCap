...
def run_drama_pipeline(
    ...
    *,
    ...
    progress("verify", "🔍 校验 scene_query + 程序拆原声段...")

    # ── 程序拆段：把「解说段里的 highlight_text」抽成独立的「原声段」 ──
    # 文案师正常写（解说段带 highlight），程序拆出原声段（narration 空 + highlight + ASR 锚定的时间）。
    # LLM 做它擅长的（写解说+选台词），程序做它擅长的（结构拆分）。
    new_segments = []
    for seg in all_segments:
        narr = (seg.get('narration_text') or '').strip()
        hl = (seg.get('highlight_text') or '').strip()
        if narr and hl:
            # 解说段里带 highlight → 拆成两段
            sq = seg.get('scene_query') or {}
            ep = sq.get('episode')
            anchored = _anchor_highlight(hl, ep, asr_texts) if ep else None
            if anchored:
                # 解说段：保留 narration，清空 highlight
                narr_seg = {**seg, 'highlight_text': ''}
                new_segments.append(narr_seg)
                # 原声段：narration 空，highlight + ASR 锚定时间
                orig_seg = {
                    'narration_text': '',
                    'highlight_text': anchored['text'],
                    'mode': 'A',
                    'scene_query': {},
                    'episode_marker': {
                        'episode': anchored['ep'],
                    ...
                    'source_start': float(anchored['start']),
                    'source_end': float(anchored['end']),
                    'video_episode': anchored['ep'],
                    'chapter_title': seg.get('chapter_title', ''),
                    'chapter_idx': seg.get('chapter_idx'),
                    'section_role': seg.get('section_role', ''),
                }
                new_segments.append(orig_seg)
            else:
                # 锚定失败（编的台词）→ 清空 highlight，保留纯解说段
                new_segments.append({**seg, 'highlight_text': ''})
        else:
            # 纯解说段 或 纯原声段 → 原样保留
            new_segments.append(seg)
    all_segments = new_segments

    # ── 原声段（无 narration）：补 source_start/end（若 scene_query 有 time_range）──
    for seg in all_segments:
        narr = (seg.get('narration_text') or '').strip()
        if narr:
            continue  # 解说段，跳过
        # 原声段：若还没有 source_start（拆段时没锚上），用 scene_query.time_range 兜底
    ...
    # ── 编排节奏校验（量化规则，0 LLM，作为结构质量信号）──
    # 方法论见 docs/tech/NARRATION_ORIGINAL_EDITING.md
    ...
    for seg in all_segments:
        ...
        if narr:
            cur_consec_narr += 1
            max_consec_narr = max(max_consec_narr, cur_consec_narr)
        else:
            cur_consec_narr = 0
            orig_count += 1
    ...
    rhythm_notes = []
    if total_segs:
        orig_ratio = orig_count / total_segs
        if orig_ratio < 0.25:
            rhythm_notes.append(f"原声占比偏低 {orig_ratio:.0%}（目标25-40%）")
        elif orig_ratio > 0.40:
            rhythm_notes.append(f"原声占比偏高 {orig_ratio:.0%}（目标25-40%）")
        if max_consec_narr > 3:
            rhythm_notes.append(f"连续纯解说 {max_consec_narr} 段（应≤3）")
        if orig_count < 4:
            rhythm_notes.append(f"原声总数 {orig_count} 段（应4-8）")
        elif orig_count > 8:
            rhythm_notes.append(f"原声总数 {orig_count} 段（应4-8）")

    if rhythm_notes:
        progress("verify",
                 f"📊 编排节奏: 原声{orig_count}/{total_segs}段({orig_ratio if total_segs else 0:.0%}) · " + "；".join(rhythm_notes),
                 {"orig_count": orig_count, "orig_ratio": round(orig_ratio, 2) if total_segs else 0})
    else:
        progress("verify",
                 f"📊 编排节奏: 原声{orig_count}/{total_segs}段 · 符合25-40%量化规则",
                 {"orig_count": orig_count, "orig_ratio": round(orig_ratio, 2) if total_segs else 0})
