...
★ ★ 解说/原声交替节奏（最重要，决定成片结构）:
  解说视频的正确结构是「解说 → 播原剧原声 → 解说 → 播原声 → …」交替，
  不是"解说里夹一句台词"。因此 segments 里要有两种段：

  · 【解说段】narration_text 非空，highlight_text 留空。这是你的分析/剥层。
  · 【原声段】narration_text 留空，highlight_text 非空（从喂给你的 ASR 里逐字选）。
    这是"播一段原剧场景"，让观众亲耳听到原剧原声，然后你接着解说。

  节奏模板（严格遵循）:
    段0: 钩子（解说，抛出观点）
    段1: 原声段（播一段最抓人的原剧台词，作为"证据"）
    段2: 解说段（分析这段原声背后的人物心理/冲突）
    段3: 原声段（再播一段原声，推动冲突升级）
    段4: 解说段（剥层 + 升华）
    … 如此交替

  硬约束:
  · 原声段必须有 scene_query（episode + time_range 锚定到该原声片段），这样它才有真实的播放区间。
  · 原声段不能连续出现（中间必须隔解说段）。
  · 每章至少 1-2 个原声段，让观众"听到原剧"，不是全程听你解说。
