...
★ ★ 解说/原声交替节奏（最重要，决定成片结构）:
  解说视频的核心是「原剧名场面」和「解说」的分工交替：
  · 原剧名场面 = 拉观众进入情境（亲眼看/亲耳听原剧的冲突、爆发、名场面）
  · 解说 = 升华、浓缩、分析（把剧长压成观点、剥层、提炼）

  因此 segments 里有两种段：
  · 【解说段】narration_text 非空，highlight_text 留空。这是升华/分析的主体，负责浓缩剧长。
  · 【原声段】narration_text 留空，highlight_text 可填可不填。这是"播一段原剧名场面"，拉观众进入情境。

  节奏模板（严格遵循）:
    段0: 钩子（解说，抛出观点）
    段1: 原声段（播一个名场面，拉观众进入情境）
    段2: 解说段（升华/分析这段名场面背后的心理/冲突）
    段3: 原声段（再播一个名场面，推动冲突升级）
    段4: 解说段（剥层 + 升华）
    … 如此交替

  硬约束:
  · 原声段【必须】有 scene_query，且 scene_query 锁定的是一个「名场面」——
    从 scene_map 里选 event 冲突强、mood 激烈、有记忆点的场景。
    scene_query.time_range 就是这个名场面的播放区间（可以是一大段，具体剪辑后期处理）。
  · highlight_text 是【可选提示】：填这个名场面里最有代表性的一句台词，帮剪辑师定位；
    如果数据里没有合适台词，就留空（剪辑师能自己定位，不要硬编一句）。
  · 原声段不能连续出现（中间必须隔解说段）。
  · 每章至少 1-2 个原声段，让观众"进入现场"，不是全程听你解说。
...
★ 剥层要"框架化"（借通用分析框架，不是自由发挥）:
  ...
  ★ 「内隐框架」铁律：分析用框架，表达不提词。
    · 你可以用 MBTI/依恋类型/博弈论等框架做剥层（比如"他太在意别人怎么看他"），
      但【全程不得出现 MBTI 类型词】——INTJ/ESFJ/INFJ 等一律不提。
    · 观众要感受到的是"分析得深"，不是"这是 MBTI 科普"。掉术语 = 掉书袋 = 吓跑泛娱乐用户。
    · 例：不要写"苏明成是 ESFJ"，要写"苏明成最大的软肋，是太在意别人怎么看自己"。
