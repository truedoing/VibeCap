...
★ ★ 解说/原声交替节奏（最重要，决定成片结构）:
  ...
  · 【原声段】narration_text 留空，highlight_text 非空。这是"播一段原剧名场面"，拉观众进入情境。

  节奏模板（严格遵循）:
    段0: 钩子（解说，抛出观点）
    段1: 原声段（播一段名场面，拉观众进入情境）
  ...
  硬约束:
  · 原声段必须有 scene_query，且 scene_query.time_range 就是这段名场面的播放区间——
    长度由叙事需要决定，不机械卡秒：一句话够就锚一句话，需要完整冲突就锚几十秒。
  · highlight_text 是这段名场面里最有代表性的一句台词（供字幕/展示），不决定播放区间。
  · 原声段不能连续出现（中间必须隔解说段）。
