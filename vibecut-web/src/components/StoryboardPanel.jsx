...
export default function StoryboardPanel({ suggestions, onSearch, curSid, curSeq }) {
  ...
  return (
    <div style={{ borderTop:'1px solid #232938', flexShrink:0, height:170, display:'flex', flexDirection:'column' }}>
      <div className="flex items-center gap-1.5 px-3 py-1.5 border-b border-border/30 text-[10px] text-muted-foreground shrink-0">
        <Sparkles size={11} className="text-purple/60" />
        <span>分镜推荐</span>
        ...
        {suggestions.map((s, i) => (
          <button key={i} onClick={() => onSearch?.(s)}
            ...
            <div className="flex items-start gap-1.5">
              <span className="text-[9px] text-purple/50 font-mono shrink-0 mt-0.5">{i+1}</span>
              <span className="text-[10px] text-foreground/75 leading-relaxed">{s}</span>
            </div>
          </button>
        ))}
      </div>
