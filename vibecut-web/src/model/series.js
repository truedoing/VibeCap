...
// ── localStorage key ──
const SERIES_INDEX_KEY = 'vibecut-series'
const taskKey = (seriesId, taskId) => `vibecut-task-${seriesId}-${taskId}`
...
export function deleteSeries(id) {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (k && k.startsWith(`vibecut-task-${id}-`)) keysToRemove.push(k)
  }
...
export function loadTasks(seriesId) {
  const prefix = `vibecut-task-${seriesId}-`
  const tasks = []
  for (let i = 0; i < localStorage.length; i++) {
    const k = localStorage.key(i)
...
// ── 兼容迁移：旧 vibecut-project → 新模型 ──
export function migrateLegacyProject() {
  try {
    const raw = localStorage.getItem('vibecut-project')
    if (!raw) return null
    ...
    saveTask(task)
    // 迁移后删除旧数据
    localStorage.removeItem('vibecut-project')
